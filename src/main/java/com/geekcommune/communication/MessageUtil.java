...
public abstract class MessageUtil {
    public static final Logger log = LogManager.getLogger(MessageUtil.class);
    ...
    protected void send(Message msg) {
        ...
        try {
            ...
            if( msg.getNumberOfTries() > MAX_TRIES ) {
                UserLog.instance().logException(e, "Failed to send message to {}", msg.getDestination().getName());
                log.error("Not retrying, exceeded max tries", e);
            } else {
                UserLog.instance().logException(e, "Failed to send message to {}, will retry", msg.getDestination().getName());
                log.error("Failed to send message to {}, will retry: {}", msg.getDestination().getName(), e.getMessage(), e);
    ...
    public void processMessage(Message msg, InetAddress inetAddress)
            throws SQLException, FriendlyBackupException {
                ...
                if( !messageHandled ) {
                    ...
                    log.debug("Unexpected message type; message: {} from inetAddress {}", msg, inetAddress);
                }
